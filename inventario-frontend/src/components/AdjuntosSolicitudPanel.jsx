...
export default function AdjuntosSolicitudPanel({ solicitudId }) {
  ...
  const cargarAdjuntos = async () => {
    try {
      const response = await api.get(`/adjuntos?solicitud_id=${solicitudId}`);
  ...
  const subirAdjunto = async (e) => {
    ...
    try {
      ...
      setArchivo(null);
      document.getElementById(`archivo-solicitud-${solicitudId}`).value = "";
